...
V_70m_2d=zeros(N,ON2); % Apparent magnitude
r_Det20m_Vlim17=zeros(3,ON2); % Object position at detection (Non inertial reference frame)
t_Obs20m_Vlim17=zeros(1,ON2); % Time from detection to impact
...
for index=1:ON
    ...
    for i=1:N2
        ...
        [V_70m_2d(i,index)]=Apparent_Magnitude(r_Obj(i,:),r_L1O(i,:));
        
        if t_Obs20m_Vlim17(index)==0
            ...
            if V_70m_2d(i,index)<=Lim_V && Epsilon>50*(pi/180) % Limiting magnitude and Sun exclusion angle
                ...
                r_Det20m_Vlim17(1,index)=r_EO(i,1)*cos(Beta)-r_EO(i,2)*sin(Beta);
                r_Det20m_Vlim17(2,index)=r_EO(i,1)*sin(Beta)+r_EO(i,2)*cos(Beta);
                r_Det20m_Vlim17(3,index)=r_EO(i,3);
...
for index2=(ON+1):ON2
    ...
    for i=1:N2
        ...
        [V_70m_2d(i,index2)]=Apparent_Magnitude(r_Obj(i,:),r_L1O(i,:));
        
        if t_Obs20m_Vlim17(index2)==0
            ...
            if V_70m_2d(i,index2)<=Lim_V && Epsilon>50*(pi/180) % Limiting magnitude and Sun exclusion angle
                ...
                % Save the position in a non inertial reference frame
                Beta=acos(dot(r_Earth(i,:),SUNL1EARTH)/(norm(r_Earth(i,:))*norm(SUNL1EARTH))); % Earth angular displacement
                r_Det20m_Vlim17(1,index2)=r_EO(i,1)*cos(Beta)-r_EO(i,2)*sin(Beta);
                r_Det20m_Vlim17(2,index2)=r_EO(i,1)*sin(Beta)+r_EO(i,2)*cos(Beta);
                r_Det20m_Vlim17(3,index2)=r_EO(i,3);
